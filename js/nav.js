...
const SIDEBAR_LINKS = [
  ...
  { href: 'snapshot.html',    label: 'Snapshot Analysis', icon: `<svg viewBox="0 0 24 24" fill="currentColor" stroke="none"><rect x="3" y="11" width="4" height="10" rx="1"/><rect x="10" y="3" width="4" height="18" rx="1"/><rect x="17" y="8" width="4" height="13" rx="1"/></svg>` },
  ...
  { href: 'about.html',       label: 'About Us',    icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="2.6" fill="currentColor" stroke="none"/></svg>` },
...
const SOCIAL_LINKS = [
  { href: 'https://orbanalytics.substack.com/', label: 'Substack', class: 'nav-substack', svg: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M22.539 8.242H1.46V5.406h21.08v2.836zM1.46 10.812V24L12 18.11 22.54 24V10.812H1.46zM22.54 0H1.46v2.836h21.08V0z"/></svg>` },
...
// ── Auth modal ────────────────────────────────────────────────────
function googleButtonHTML(tab) {
  return `
    <button type="button" onclick="handleGoogleSignIn('${tab}')"
      style="width:100%;padding:0.65rem;background:var(--surface);color:var(--text);border:1px solid var(--border);
             border-radius:8px;font-family:var(--font-body);font-size:0.9rem;font-weight:600;cursor:pointer;
             transition:background 0.2s;display:flex;align-items:center;justify-content:center;gap:0.6rem;margin-bottom:1rem"
      onmouseover="this.style.background='var(--hover-tint)'"
    ...
    <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:1.25rem">
      ...
      <span style="font-size:0.72rem;color:var(--gray-500);text-transform:uppercase;letter-spacing:0.05em">or</span>
      <div style="flex:1;height:1px;background:var(--border)"></div>
    </div>
  `;
}
...
window.handleGoogleSignIn = async function(tab) {
  const errEl = document.getElementById(tab === 'signup' ? 'signup-error' : 'signin-error');
  errEl.style.display = 'none';
  try {
    await window.orbSignInWithGoogle();
    closeAuthModal();
  } catch(e) {
...
function friendlyError(code) {
  const map = {
    ...
    'auth/account-exists-with-different-credential': 'An account already exists with this email. Please sign in with your original method instead.',
  };
    ...
    topNav.innerHTML = `
      ...
      <div style="display:flex;align-items:center;gap:0.4rem;margin-left:auto;flex-shrink:0;z-index:1">
        <div class="nav-social">
          ${SOCIAL_LINKS.map(s => `
            <a href="${s.href}" target="_blank" rel="noopener noreferrer" aria-label="${s.label}" title="${s.label}" class="${s.class || ''}">${s.svg}</a>
          `).join('')}
        </div>
